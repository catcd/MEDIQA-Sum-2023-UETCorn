...
python main.py -p "summ-complete" \
      ...
      -o "run_results/$prefix-raw-results_run1.csv" \
      -ic "ID" \
      -hc "section_header" \
      -dc "dialogue" \
      -pc "config/config-v21.json"
...
python main.py -i "run_results/$prefix-raw-results_run1.csv"   \
  -o "taskB_uetcorn_run1_mediqaSum.csv" \
  -dc "SystemOutput" \
  -ic "TestID" \
  -p "postprocessing" \
  -pc "config/config-v21.json"
