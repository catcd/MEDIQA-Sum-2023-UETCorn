...
python -m venv --clear uetcorn_taskB_venv
# Python 3.8.5
...
. uetcorn_taskB_venv/bin/activate
pip install -U pip setuptools wheel
pip install nvidia-pyindex
pip install nvidia-tensorrt==7.2.3.4
python -c "import tensorrt; print(tensorrt.__version__); assert tensorrt.Builder(tensorrt.Logger())"
echo "export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:$(pwd)/uetcorn_taskB_venv/lib/python3.8/site-packages/tensorrt/:$(pwd)/uetcorn_taskB_venv/lib/python3.9/site-packages/tensorrt/:$(pwd)/uetcorn_taskB_venv/lib/python3.10/site-packages/tensorrt/" >> uetcorn_taskB_venv/bin/activate
deactivate
...
python -c "from transformers import pipeline; _ = pipeline('question-answering', model='distilbert-base-cased-distilled-squad', tokenizer='distilbert-base-cased-distilled-squad', device='cpu')"
